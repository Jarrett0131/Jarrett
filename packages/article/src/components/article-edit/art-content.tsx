...
import { useNavSubmitting } from '@shared/utils/hooks.ts';
import { useSubmit} from 'react-router-dom';
import ReactQuill from 'react-quill';
import 'react-quill/dist/quill.snow.css';
import styles from '../article-add/css/art-content.module.less';
import useArticleEditStore, { updateCurrent, setContent, setArticleState, selectContent ,selectIsShowDraft } from '@article/store/art-edit-store';
import { Move } from '@article/store/art-add-store';
import { modules } from '@article/components/article-add/art-content';
      ...
      <Spin spinning={submitting} delay={200}>
        <Space direction="vertical" style={{ display: 'flex' }}>
          ...
          <Space direction="horizontal">
            ...
            type="primary" 
            onClick={() => publish('草稿')}
            style={{ display: isShowDarft.current ? '' : 'none' }} 
            >
              存为草稿
            </Button>
