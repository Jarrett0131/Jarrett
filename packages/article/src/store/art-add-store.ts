...
import { createStorage } from '@shared/utils/storage.ts';
...
export const setArticleAttachment = (url: string, hash: string) => {
  useArtAddStore.setState((state) => {
    state.article.attachment_url = url;
    state.article.attachment_hash = hash;
  })
};
